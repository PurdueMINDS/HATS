...
#         print output_intra.size()
        #pdb.set_trace()
        #intra_attn_vectors =    output_intra[-1,:,:].squeeze()
...
class InterRNN(nn.Module):
    ...
    def forward(self, intra_attention_vectors, state_inter):
        ...
        #pdb.set_trace()
        #final_map = self.final_linear(output_inter[-1,:,:].squeeze())
        #pdb.set_trace()
        return output_inter[-1,:,:].squeeze() #, state_inter, inter_attn_norm
...
class TextModels(nn.Module):
    ...
    def __init__(self, vocab_size, input_dim, model, num_layers, num_neurons, janossy_k,janossy_k2, device):
        ...
        if self.model_name == 'lstm':
            self.model = nn.LSTM(1, 50,num_layers = 2, batch_first=True)#, bidirectional=True)
            self.model_activation = None
            self.model_out_shape = 50
        elif self.model_name == 'gru':
            gru_size = self.original_input_dim * janossy_k * janossy_k2
            self.model = nn.GRU( 1, 80, batch_first=True)
            #pdb.set_trace()
            self.model_activation = None
            ...
            small_hidden_size = 8
            small_n_layers = 2
            self.intra_attn = AttentionIntraRNN(batch_size=128, num_tokens=10, embed_size=1, intra_gru_hidden=20, bidirectional= True)
            self.intra_attn2 = AttentionIntraRNN(batch_size=128, num_tokens=10, embed_size=1, intra_gru_hidden=20, bidirectional= True)         
            ...
            small_hidden_size = 8
            small_n_layers = 2
            self.intra_attn = IntraRNN(batch_size=128, num_tokens=10, embed_size=1, intra_gru_hidden=20, bidirectional= True)       
    ...
    def forward(self, input_tensor):
        ...
        if self.model_activation is not None:
            ...
            if self.model_name == 'hier':
                ...
                mini_batch = mini_batch.view(mini_batch.shape[0],mini_batch.shape[1],mini_batch.shape[2],mini_batch.shape[3],1)
                ...
                for i in xrange(max_inters):
                    _st = None
                    for j in xrange(mini_batch.shape[2]):
                        _sttmp = self.intra_attn(mini_batch[:,i,j,:].transpose(0,1), state_intra)
                        if _st is None:
                            _st = _sttmp
                        else:
                            _st =_st +  _sttmp
                ...
                emb_output = emb_output.view(emb_shape[0], emb_shape[1]*emb_shape[2], -1)
                model_out, _ = self.model(emb_output)
                model_out = model_out[:, -1, :]  # Just the final state
        if self.model_name in ['lstm','gru','cnn','hats','hier']:
            rho_out = model_out
        else:
